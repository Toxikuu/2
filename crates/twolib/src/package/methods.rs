...
impl Package {
    pub fn from_both(repo: &str, name: &str) -> Self { Self::_create(repo, name) }

    pub fn from_relpath(relpath: &str) -> Self {
        let (repo, name) = relpath
            .split_once('/')
            .efail(|| format!("Improperly formatted relpath: '{relpath}'"));

        Self::_create(repo, name)
    }

    pub fn from_name(name: &str) -> Self {
        let repo = ambiguity::resolve_ambiguity(name);
        Self::_create(&repo, name)
    }

    /// # Description
    /// Creates a package given its repo and name
    fn _create(repo: &str, name: &str) -> Self {
